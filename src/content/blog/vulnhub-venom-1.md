...
summary: 'Writeup of vulnhub venom 1, as 113-1 ISAD WEEK6 Assignment'
date: 2024-11-16
---
...
1. 準備 Kali Linux 虛擬機（以下簡稱 Kali）：
    ...
    - 開啟 Kali
    <img src="/blog/vulnhub-venom1/KaliTurnOn.png" alt="Turn on Kali" width="70%" />

2. 準備 Venom 虛擬機（以下簡稱 Venom）
    - 下載 Venom.zip 檔案：https://download.vulnhub.com/venom/venom.zip
    <img src="/blog/vulnhub-venom1/VenomDownload.png" alt="Download Venom.zip file" width="70%" />
    - 下載完成可以跑 MD5 和 SHA1 確認檔案下載完整
    <img src="/blog/vulnhub-venom1/VenomIntegrityCheck.png" alt="Integrity Check of Venom.zip" width="100%" />
    - 匯入 VMware
    - 設定網路：對 Venom 點擊右鍵 > Settings > Network Adapter > **NAT** > OK
    <img src="/blog/vulnhub-venom1/VenomNetworkConfig.png" alt="Network Configuration of Venom" width="70%" />
    - 開啟 Venom
    <img src="/blog/vulnhub-venom1/VenomTurnOn.png" alt="Turn on Venom" width="70%" />
3. 觀察網路資訊
    - 檢視本機網路 (`ipconfig`)：與 VMware 有關的 IP 為 192.168.119.1 與 192.168.225.1
    <img src="/blog/vulnhub-venom1/HostNetwork.png" alt="Host Network" width="70%" />
    - 在 Kali 檢視虛擬機網路 (`ip a`)：IP 為 192.168.225.131
    <img src="/blog/vulnhub-venom1/KaliNetwork.png" alt="Kali Network" width="100%" />
    - 比對可知，虛擬機 (Kali & Venom) IP 位置應為 192.168.225.0/24

## Step 1. 尋找可用服務（入口）

1. 在 Kali 上針對 192.168.225.0/24 進行掃描，得知 Venom IP 位置應為 192.168.225.132
    <img src="/blog/vulnhub-venom1/nmapIp.png" alt="nmap IP" width="70%" />
2. 針對 192.168.225.132 進行掃描，觀察開啟的服務
    <img src="/blog/vulnhub-venom1/nmapPort.png" alt="nmap port" width="70%" />
    - FTP: 21
    - HTTP: 80
    - HTTPS: 443
...
1. 檢視 HTTP 服務相關線索：使用 `curl 192.168.225.132:80`
    <img src="/blog/vulnhub-venom1/FlagInHttpPage.png" alt="HTTP Page" width="100%" />
    - 發現最後有一行註解掉的線索：``
2. 將線索丟到 crackstation 解密，得知該線索為 `hostinger` MD5 雜湊值，推測 `hostinger` 為 password
    <img src="/blog/vulnhub-venom1/MD5Decrypt.png" alt="MD5 Decrypt" width="100%" />

## Step 3. FTP

1. 由前一步得知的訊息，使用 FTP 登入
    <img src="/blog/vulnhub-venom1/ftp.png" alt="FTP" width="100%" />
...
2. 打開 `cat hint.txt` 檔案觀察其內容
    <img src="/blog/vulnhub-venom1/CatHint.png" alt="cat hint.txt" width="100%" />
...
1. 去 https://cryptii.com/pipes/vigenere-cipher 進行解密，得到密碼為 `E7r9t8@Q#h%Hy+M1234`
    <img src="/blog/vulnhub-venom1/VigenereCipherDecrypt.png" alt="standard vigenere cipher" width="70%" />
2. 透過密碼登入 venom.box
    - 為了連上 http://venom.box，需先在 /etc/hosts 新增 domain name
        <img src="/blog/vulnhub-venom1/AddDomainNameInEtcHosts.png" alt="Add domain name in /etc/hosts" width="70%" />
    - 連上 venom.box 並登入，確認為 administrator 權限 
        <img src="/blog/vulnhub-venom1/VenomLogIn.png" alt="Venom Login" width="100%" />
...
1. 檢視是否有存在漏洞
    - 使用 `searchsploit subrion cms 4.2.1` 指令
        <img src="/blog/vulnhub-venom1/ExploitInSubrion.png" alt="Search for exploit" width="100%" />
        - 看到其中一個漏洞為可上傳任意檔案
2. 準備 reverse shell
    - 使用 https://revshells.com 產生 reverse shell
    <img src="/blog/vulnhub-venom1/RevShellGenerator.png" alt="Generate a reverse shell" width="100%" />
...
3. 上傳 reverse shell
    - 在 http://venom.box/ 上點擊齒輪圖示進到管理者頁面 (http://venom/box/panel)，點擊右方 content > uploads 進到上傳區
        <img src="/blog/vulnhub-venom1/VenomUpload.png" alt="Go to upload" width="100%" />
    - 上傳準備好的 reverse shell `rev.phar`
        <img src="/blog/vulnhub-venom1/UploadRevShell.png" alt="Upload reverse shell" width="100%" />
4. 取得 reverse shell
    - 在另一個終端機上透過指令 `nc -nlvp 6666` 監聽 port 6666
        <img src="/blog/vulnhub-venom1/Listening.png" alt="Listening port 6666" width="100%" />
    - 開新分頁 http://venom.box/uploads/rev.phar 執行 web shell
    - 終端機上成功獲取 reverse shell
        <img src="/blog/vulnhub-venom1/GetReverseShell.png" alt="Get reversse shell" width="100%" />

## Step 6. 取得 root shell

1. 嘗試使用 `sudo -i` 指令切換為 root 身分，發現因為沒有 tty 所以無法執行，這樣會沒辦法拿到 root 權限
    <img src="/blog/vulnhub-venom1/SudoFail.png" alt="Sudo Fail" width="100%" />
2. 使用 `python -c 'import pty; pty.spawn("/bin/bash");'` 將 shell 切換成 pty shell
    <img src="/blog/vulnhub-venom1/GetPTYShell.png" alt="Get pty shell" width="100%" />
3. 再次使用 `sudo -i` 指令嘗試切換為 root 身分，但無從得知 www-data 的密碼
4. 查看系統內有哪些 user，發現有 hostinger 和 nathan 兩個
    <img src="/blog/vulnhub-venom1/CheckAllUser.png" alt="Check all user" width="100%" />
5. 將 user 切換為 `hostinger`，同樣嘗試執行 `sudo -i` 指令，發現 `hostinger` 沒有權限
    <img src="/blog/vulnhub-venom1/HostingerSudoFail.png" alt="Hostinger sudo fail" width="100%" />
6. 登入 Nathan 帳戶
    - 首先應該找到 Nathan 的密碼，使用 `locate .htaccess` 可以看到以下可能檔案
    <img src="/blog/vulnhub-venom1/locateHtaccess.png" alt="Locate .htaccess" width="70%" />
    - 打開 `/var/www/html/subrion/backup/.htaccess`，看到一串密文，應該為 Nathan 的密碼
    <img src="/blog/vulnhub-venom1/catNathanPassword.png" alt="Get Nathan Password" width="100%" />
    - 使用指令 `su nathan` 切換 user 為 Nathan
    <img src="/blog/vulnhub-venom1/GetNathanShell.png" alt="Get Nathan Shell" width="100%" />
7. 取得 root shell
    - 使用 `sudo -i` 指令切換為 root 身分
    <img src="/blog/vulnhub-venom1/GetRootShell.png" alt="Get root shell" width="100%" />
    - 進到 `/root`，打開 `root.txt` 檔案，得到 flag
    ![Get flag](/blog/vulnhub-venom1/GetFlag.png)
    <img src="/blog/vulnhub-venom1/GetFlag.png" alt="Get flag" width="100%" />
